...
// Patch only the old scenic placement expression; unit-tested geometry clamps all edges.
let hq=read('public/hq.js');
const oldTransform='stage.style.transform=`translate(-50%,-50%) translate(${-(x-.5)*w*z}px,${-(y-.5)*h*z}px) scale(${z})`;';
