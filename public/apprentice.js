...
import { caseView,lessonsView } from './apprentice-case.js';
{
 ...
 function paint(){if(!ap.active||!state?.apprenticeship)return;const s=state.apprenticeship;
  area.innerHTML=top()+(ap.caseId&&ap.detail?caseView(ap.detail,team()):ap.tab==='phone'?phoneView(s,ap.cases):ap.tab==='lessons'?lessonsView(ap.lessons):ap.tab==='observations'?observationView(s,ap.cases):caseList(ap.cases,ap.more,ap.role));
 }
