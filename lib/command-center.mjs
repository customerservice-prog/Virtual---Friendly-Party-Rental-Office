...
export class CommandCenter {
  ...
  submit(b){
    ...
    if(action==='brief'){
      const lower=message.toLowerCase(),asks=/\b(check|review|inspect|investigate|look\s+(?:at|into)|verify|find\s+out|what(?:'s| is)\s+wrong)\b/i.test(message);
      if(asks&&/\b(website|site|checkout|page|pages|deploy|deployment|broken link|web)\b/i.test(lower))action='website';
      else if(asks&&/\b(order|orders|delivery|deliveries|pickup|pickups|balance|booking|bookings|inventory|availability|schedule)\b/i.test(lower))action='orders';
    }
    if(!['brief','website','orders'].includes(action))throw new OfficeError('Choose an office brief, website check or order review.');
  ...
  async directChat(job,signal){
    ...
    const history=detail.notes.filter(n=>n.kind==='owner-request'||n.kind==='summary').slice(-16).map(n=>({author:n.author,body:n.body}));
