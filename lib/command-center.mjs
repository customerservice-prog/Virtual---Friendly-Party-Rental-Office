...
export class CommandCenter {
  ...
  card(role) {
    ...
    else if(live?.role===role){status='working';activity=live.label||'Working on the current conversation.';caseId=live.caseId;}
  ...
  huddle(){
    const latest=this.d.db.prepare("SELECT n.case_id,MAX(n.id) latest,COUNT(*) n FROM ap_notes n JOIN ap_cases c ON c.id=n.case_id WHERE n.kind IN ('plan','question','contribution','finding') AND c.status IN ('queued','working') GROUP BY n.case_id HAVING COUNT(*)>=2 ORDER BY latest DESC LIMIT 1").get();
    if(!latest)return null;
    const rows=this.d.db.prepare("SELECT n.*,c.title,c.practice,c.status FROM ap_notes n JOIN ap_cases c ON c.id=n.case_id WHERE n.case_id=? AND n.kind IN ('plan','question','contribution','finding') ORDER BY n.id DESC LIMIT 12").all(latest.case_id);
    if(rows.length<2)return null;
    const messages=rows.map(n=>{const body=this.d.unpack(n.body);return {id:n.id,caseId:n.case_id,title:this.d.unpack(n.title),practice:!!n.practice,caseStatus:n.status,author:n.author,recipient:n.recipient,kind:n.kind,body:body.slice(0,1600),truncated:body.length>1600,at:n.created};});
    const participants=[...new Set(messages.flatMap(n=>[n.author,n.recipient]).filter(id=>TEAM.some(r=>r.id===id)))];
    return {caseId:latest.case_id,title:messages[0]?.title||'Team huddle',caseStatus:messages[0]?.caseStatus||null,participants,messages};
  }
  snapshot(){
    const team=TEAM.map(r=>this.card(r.id)),huddle=this.huddle(),participants=new Set(huddle?.participants||[]);
    const scene=team.map(r=>({role:r.id,place:participants.has(r.id)?'huddle':r.status==='review'?'owner':r.id==='phone'&&r.status==='working'?'phone':r.id==='email'&&r.status==='working'?'inbox':r.id==='tech'&&r.status==='working'?'website':'desk',state:r.status,activity:r.activity,caseId:r.caseId||null}));
    return {settings:this.settings(),heartbeat:this.heartbeat,startedAt:this.startedAt,serverTime:stamp(),team,scene,counts:this.counts(),feed:this.feed(),huddle,attention:this.attention(),
  ...
  submit(b){
    ...
    this.worker.tick();this.s.changed();return {caseId:c.id,queued:true,paused:!!this.s.get('settings').paused,action,lead:recipient==='team'?(action==='website'?'tech':action==='email'?'email':action==='phone'?'phone':'office'):recipient};
  }
  ...
  recentOwnerMemory(excludeId){
    const rows=this.d.db.prepare(`SELECT n.author,n.kind,n.body,n.created,n.case_id FROM ap_notes n JOIN ap_cases c ON c.id=n.case_id WHERE n.kind IN ('owner-request','summary') AND c.status!='deleted' AND n.case_id!=? AND EXISTS(SELECT 1 FROM ap_sources s WHERE s.case_id=n.case_id AND s.kind='owner') ORDER BY n.id DESC LIMIT 8`).all(excludeId);
    return rows.reverse().map(n=>({author:n.author,body:'[Earlier office conversation] '+this.d.unpack(n.body).slice(0,420)}));
  }
  shouldDirectChat(detail,job){
    const owner=detail.sources.filter(s=>s.kind==='owner').at(-1),p=owner?.payload||{};
    const message=String(p.message||'').trim();
    if(!message)return false;
    if(/\b(work together|everyone (?:look|check|review|figure)|team (?:look|check|review|figure)|huddle|all of you|all four)\b/i.test(message))return false;
    return true;
  }
  async directChat(job,signal){
    ...
    const p=owner.payload,actionLead=p.action==='website'?'tech':p.action==='email'?'email':p.action==='phone'?'phone':'office',lead=p.recipient==='team'?actionLead:p.recipient;
    const label=p.action==='email'?'Reading the mailbox and replying to Bryan…':p.action==='phone'?'Checking phone activity and replying to Bryan…':p.action==='website'?'Checking the site and replying to Bryan…':p.action==='orders'?'Checking live orders and replying to Bryan…':'Thinking about Bryan’s message…';
    this.worker.live={caseId:job.case_id,role:lead,lead,kind:'chat',label,startedAt:stamp()};this.s.changed();
    ...
    history=history.slice(-8).map(n=>({author:n.author,body:n.body}));
    history=[...this.recentOwnerMemory(job.case_id),...history].slice(-12);
    const source=detail.sources.filter(s=>s.kind==='authorized-check').at(-1);
    const officeContext={...this.context(),requestedAction:p.action,...(source?{sourceCheck:{observedAt:source.updated,content:String(source.payload?.message||'').slice(0,6500)}}:{})};
    const reply=await this.i.chat({role:lead,recipient:p.recipient,message:p.message,history,officeContext},signal);
