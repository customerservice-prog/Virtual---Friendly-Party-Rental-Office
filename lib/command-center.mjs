...
export class CommandCenter {
  constructor(ap) {
    ...
    if(!this.s.get('command:settings'))this.s.set('command:settings',{enabled:true,website:true,orders:true});
  ...
  async tick(force=false){
    ...
    const promise=(async()=>{
      ...
      if(force||!q||Date.parse(q.nextAt)<=now){
        const counts=this.counts();this.s.set('command:duty:queue',{lastAt:stamp(),nextAt:new Date(now+60000).toISOString(),status:'checked',detail:'Reviewed saved queue, blockers and owner-review counts. No customer records inferred.',counts});
      }
      for(const id of ['website','orders']){
        ...
        const nextAt=new Date(now+30*60000).toISOString();
