...
export class CommandCenter {
  ...
  huddle(){
    const latest=this.d.db.prepare("SELECT n.case_id,MAX(n.id) latest,COUNT(*) n FROM ap_notes n JOIN ap_cases c ON c.id=n.case_id WHERE n.kind IN ('plan','question','contribution','finding') AND c.status!='deleted' GROUP BY n.case_id HAVING COUNT(*)>=2 ORDER BY latest DESC LIMIT 1").get();
  ...
  snapshot(){return {settings:this.settings(),heartbeat:this.heartbeat,startedAt:this.startedAt,serverTime:stamp(),team:TEAM.map(r=>this.card(r.id)),counts:this.counts(),feed:this.feed(),huddle:this.huddle(),attention:this.attention(),
