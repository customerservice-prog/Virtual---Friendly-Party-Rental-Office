    ...
    try{
      ...
      for(const raw of box.messages||[]){
        ...
        const msg={messageId:String(raw.id),threadId:threadSeed,subject,from:String(raw.from||'').slice(0,300),to:Array.isArray(raw.to)?raw.to.join(', ').slice(0,500):String(raw.to||'').slice(0,500),
          message:redact(String(raw.message||'').slice(0,14000)),direction:raw.direction==='sent'?'sent':'incoming',labels:[String(raw.folder||'')],unread:!!raw.unread,automated:/no.?reply|mailer-daemon|notification|automated/i.test(String(raw.from||'')),humanConfirmed:false,
