...
export function classifyCustomerMail(subject,message){const text=(String(subject)+' '+String(message)).toLowerCase();if(/refund|cancel|cancellation|complaint|angry|upset|damag|missing|never (got|received)|didn.t receive|chargeback/.test(text))return {category:'Customer issue',priority:'high'};if(/balance|payment|paid|deposit|card|receipt|invoice/.test(text))return {category:'Payment / balance',priority:'normal'};if(/order\s*#|order number|delivery|pickup|existing order/.test(text))return {category:'Existing order',priority:'normal'};if(/wedding|ceremony|reception/.test(text))return {category:'Wedding inquiry',priority:'normal'};if(/quote|availability|tent|table|chair|bounce|waterslide|rental/.test(text))return {category:'New inquiry',priority:'normal'};return {category:'Customer email',priority:'normal'};}
export function decodeMail(m,includeDrafts=false) {
  ...
  const subject=h('subject').slice(0,160)||'Untitled message',classification=classifyCustomerMail(subject,message);
  return {messageId:m.id,threadId:m.threadId,subject,from:h('from').slice(0,300),to:h('to').slice(0,500),
    message:redact(message.slice(0,14000)),direction,labels,unread:labels.includes('UNREAD'),automated,humanConfirmed:false,category:classification.category,priority:classification.priority,locationReviewNeeded:/friendlypartyrentalsc\.com|south carolina|\[SC\]/i.test(subject+' '+message),at:new Date(timestamp).toISOString(),
...
export class MailObserver {
  ...
  status(){const s=this.store.get('ap:mail')||{};return {account:ACCOUNT,enabled:this.data.settings().observing,connected:this.i.status().gmail.connected,
    configured:this.i.status().gmail.configured,connectionMode:this.i.status().gmail.mode||null,phase:s.phase||'not-started',lastAttempt:s.lastAttempt||null,lastSync:s.lastSync||null,
  ...
  async runRelay(signal){
    ...
    try{
      ...
      for(const raw of box.messages||[]){
        ...
        if(!current)continue;
        const classification=classifyCustomerMail(subject,raw.message||'');
        const msg={messageId:String(raw.id),threadId:threadSeed,subject,from:String(raw.from||'').slice(0,300),to:Array.isArray(raw.to)?raw.to.join(', ').slice(0,500):String(raw.to||'').slice(0,500),
          message:redact(String(raw.message||'').slice(0,14000)),direction:raw.direction==='sent'?'sent':'incoming',labels:[String(raw.folder||'')],unread:!!raw.unread,automated:/no.?reply|mailer-daemon|notification|automated/i.test(String(raw.from||'')),humanConfirmed:false,category:classification.category,priority:classification.priority,
        ...
        if(changed&&msg.direction==='incoming'&&msg.priority==='high')this.data.note(current.id,'email','owner','flag',`Avery classified this as ${msg.category}. It is queued for review; no refund, cancellation or customer promise has been made.`,this.data.sources(current.id).filter(s=>s.kind==='email').slice(-1).map(s=>s.id));
        if(changed&&this.data.settings().autoReview&&msg.direction==='incoming'&&msg.unread&&!msg.automated&&!msg.locationReviewNeeded){
