...
export class ApprenticeData {
  constructor(store,integrations) {
    ...
    this.db.exec(`CREATE TABLE IF NOT EXISTS ap_cases(id TEXT PRIMARY KEY,source_key TEXT UNIQUE,title TEXT NOT NULL,
      ...
      CREATE TABLE IF NOT EXISTS phone_calls(id TEXT PRIMARY KEY,provider_call_id TEXT UNIQUE,state TEXT NOT NULL,payload TEXT NOT NULL,created TEXT NOT NULL,updated TEXT NOT NULL);
      CREATE INDEX IF NOT EXISTS phone_calls_updated ON phone_calls(updated);
    ...
    if(!store.get('phone:settings')) store.set('phone:settings',{businessName:'Friendly Party Rental',carrierAttached:false,carrierName:null,mainNumber:null,businessHours:'Mon-Sat 9:00 AM-6:00 PM ET',businessRoute:['101','102','199'],afterHoursRoute:['102','199'],recording:false,aiAnswering:false,extensions:[{extension:'100',name:'Bryan',role:'Owner'},{extension:'101',name:'Nicole',role:'Office / phone trainer'},{extension:'102',name:'Riley',role:'AI receptionist'},{extension:'199',name:'Voicemail',role:'Voicemail'}],lastCarrierEvent:null});
  ...
  phoneSettings(){return this.store.get('phone:settings');}
  phoneCalls(limit=30){return this.db.prepare('SELECT * FROM phone_calls ORDER BY updated DESC LIMIT ?').all(Math.max(1,Math.min(100,limit))).map(r=>({...r,payload:this.unpack(r.payload)}));}
  updatePhoneSettings(patch){
    const old=this.phoneSettings(),next={...old};
    if(patch.businessHours!==undefined)next.businessHours=requireText(patch.businessHours,'Business hours',120);
    for(const key of ['businessRoute','afterHoursRoute']){
      if(patch[key]!==undefined){
        if(!Array.isArray(patch[key])||patch[key].length<1||patch[key].length>8)throw new OfficeError(key+' needs 1–8 extensions.');
        const valid=new Set(old.extensions.map(x=>x.extension));if(patch[key].some(x=>!valid.has(String(x))))throw new OfficeError('Routing contains an unknown extension.');
        next[key]=patch[key].map(String);
      }
    }
    if(patch.recording!==undefined){if(typeof patch.recording!=='boolean')throw new OfficeError('recording must be true or false.');next.recording=patch.recording;}
    if(patch.aiAnswering!==undefined){if(typeof patch.aiAnswering!=='boolean')throw new OfficeError('aiAnswering must be true or false.');if(patch.aiAnswering&&!old.carrierAttached)throw new OfficeError('Attach and verify a phone carrier before enabling AI answering.',409);next.aiAnswering=patch.aiAnswering;}
    this.store.set('phone:settings',next);this.store.changed();return next;
  }
  recordPhoneEvent(event){
    this.needPrivate();
    const id=requireText(event.eventId||event.callId||randomUUID(),'Phone event id',200),providerId=requireText(event.callId||id,'Provider call id',200);
    const state=requireText(event.state||event.type||'event','Call state',80);
    const payload=JSON.parse(JSON.stringify({...event,receivedAt:stamp()},(k,v)=>typeof v==='string'?redact(v):v));
    const now=stamp(),packed=this.pack(payload,false);
    this.db.prepare('INSERT INTO phone_calls(id,provider_call_id,state,payload,created,updated) VALUES(?,?,?,?,?,?) ON CONFLICT(provider_call_id) DO UPDATE SET state=excluded.state,payload=excluded.payload,updated=excluded.updated').run(digest('phone-event:'+id),providerId,state,packed,now,now);
    const old=this.phoneSettings();this.store.set('phone:settings',{...old,carrierAttached:true,carrierName:String(event.provider||old.carrierName||'Connected carrier').slice(0,100),mainNumber:event.to||old.mainNumber||null,lastCarrierEvent:now});
    this.store.changed();return {providerCallId:providerId,state,receivedAt:now};
  }
