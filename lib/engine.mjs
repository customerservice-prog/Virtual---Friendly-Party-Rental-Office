...
export function reviewOrders(snapshot) {
  const lines=[`ORDER READINESS REVIEW\nSource timestamp: ${snapshot.asOf}\nThis report describes the supplied snapshot, not a new reservation or inventory guarantee.`];
  ...
  if(!snapshot.orders.length) lines.push('\nThe supplied snapshot contains no orders. This does not establish that the business has no bookings.');
  for(const o of snapshot.orders) {
    ...
    else if(o.balanceDue>0) gaps.push(`source lists $${Number(o.balanceDue).toFixed(2)} balance due; verify payment status`);
    for(const i of o.items) if(i.availability!=='confirmed') gaps.push(`${i.name}: availability ${i.availability||'unconfirmed'}`);
    if(!o.items.length) gaps.push('item list empty');
    lines.push(`\n${o.id}\n${gaps.length ? gaps.map(g=>`• ${g}`).join('\n') : 'Required fields are present in this snapshot; physical inventory, crew, access and travel still need verification.'}\nItems: ${o.items.map(i=>`${i.quantity} × ${i.name}`).join('; ')||'None provided'}`);
  }
  lines.push('\nOWNER CHECKS\nResolve missing details and confirm equipment, payment, access and crew readiness in the authoritative system. No order or customer message was changed.');
  return lines.join('\n');
}
