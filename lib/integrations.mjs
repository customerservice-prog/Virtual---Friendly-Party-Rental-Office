...
export class Integrations {
  ...
  status() {
    ...
    return {
      ai: { configured: !!((e.AI_BRAIN_URL && e.AI_BRAIN_MODEL) || (e.OPENAI_API_KEY && e.OPENAI_MODEL)), model: e.AI_BRAIN_MODEL || e.OPENAI_MODEL || null, chatModel: e.AI_CHAT_MODEL || e.AI_BRAIN_MODEL || e.OPENAI_MODEL || null, provider: (e.AI_BRAIN_URL && e.AI_BRAIN_MODEL) ? 'Friendly private brain' : (e.OPENAI_API_KEY && e.OPENAI_MODEL) ? 'OpenAI' : null, verified: this.store.get('verified:ai'), limit: this.aiLimit() },
  ...
  async chat({role='office',recipient='team',message,history=[],officeContext={}},signal) {
    const chatUrl=this.env.AI_CHAT_URL||this.env.AI_BRAIN_URL,chatModel=this.env.AI_CHAT_MODEL||this.env.AI_BRAIN_MODEL;
    if(!(chatUrl&&chatModel)) throw new OfficeError('The private Friendly AI brain is not available right now.');
    ...
    const procedures=this.store.rules().filter(r=>r.status==='approved').slice(0,12).map(r=>`- ${String(r.title).slice(0,100)}: ${String(r.body).slice(0,420)}`).join('\n').slice(0,4200);
...
APPROVED FRIENDLY PROCEDURES (use only when relevant; if absent, do not invent policy):
${procedures||'No approved reusable procedures were supplied.'}`;
    ...
    try{
      const r=await fetch(new URL('/api/chat',u),{method:'POST',signal:controller.signal,headers:{'content-type':'application/json'},body:JSON.stringify({
        model:chatModel,stream:false,keep_alive:-1,messages,
        options:{temperature:0.55,top_p:0.9,num_ctx:3072,num_predict:320,repeat_penalty:1.05}
      })});
