...
export class Apprenticeship {
  ...
  summary(){return {settings:this.data.settings(),mail:this.mail.status(),counts:this.data.counts(),live:this.worker.live,
    ...
    phone:{configured:!!(this.i.env.PHONE_WEBHOOK_SECRET?.length>=32),enabled:this.data.settings().phoneWebhook,lastReceived:this.store.get('ap:phoneReceived'),liveListening:false},
    storageReady:!!this.i.env.INTEGRATION_ENCRYPTION_KEY,modelReady:this.i.status().ai.configured,transcriptionReady:!!(this.i.env.OPENAI_API_KEY&&this.i.env.OPENAI_TRANSCRIBE_MODEL),businessActionsEnabled:false};}
  ...
  async route(method,path,url,b={}){
    ...
    const cm=path.match(/^\/api\/apprentice\/cases\/([a-f0-9-]{36})\/(review|note|lesson|approve|delete|confirm-sent|edit-transcript|cancel)$/);
