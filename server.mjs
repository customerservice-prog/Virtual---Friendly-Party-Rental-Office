...
export function createOffice({env=process.env,store=new Store(resolve(env.DATA_DIR||'data','office.sqlite')),integrations,worker=true}={}) {
  ...
  async function body(req) {
    ...
    for await(const chunk of req) { size+=chunk.length; if(size>90000) throw new OfficeError('Request too large.',413); chunks.push(chunk); }
    try { const value=JSON.parse(Buffer.concat(chunks).toString('utf8')||'{}'); if(!value || Array.isArray(value) || typeof value!=='object') throw new Error(); return value; } catch { throw new OfficeError('Invalid JSON object.'); }
  }
  const server=http.createServer(async(req,res)=>{
    ...
    try {
      ...
      if(path.startsWith('/api/')) {
        ...
        if(path==='/api/state' && req.method==='GET') { output(res,200,{...store.snapshot(),integrations:integrations.status(),csrf:auth.csrf,localAccess:!expected});return; }
        ...
        if(path==='/api/settings') {
          ...
          if(b.mode!==undefined) {
            ...
            if(engine.active.size || imports.size) throw new OfficeError('Pause and allow in-progress requests to settle before changing modes.',409);
            next.mode=b.mode;
          }
          ...
          if(b.paused===true) {engine.pauseAll();for(const c of imports)c.abort();}
        ...
        if(path==='/api/agents') {
          ...
          store.set(`agent:${b.role}`,{paused:b.paused});if(b.paused)engine.pauseAgent(b.role);
        ...
        if(path==='/api/integrations/gmail/disconnect') { integrations.disconnectGmail();output(res,200,{ok:true});return; }
      ...
      const files={'/':'index.html','/index.html':'index.html','/app.js':'app.js','/scene.js':'scene.js','/style.css':'style.css','/icon.svg':'icon.svg'};
  ...
  return {server,store,engine,integrations,async close(){clearInterval(cleanup);clearTimeout(timer);for(const c of imports)c.abort();for(const stream of streams)stream.res.end();await engine.stop();await new Promise(r=>server.close(r));store.close();}};
}
